...
void *opaque_qxl = NULL;
uint8_t *qxl_ram_base = NULL;
                             ...
                             uint32_t val, int len)
{
    ...
    if (qxl->id == 0) {
      ...
      vga_dirty_log_stop((VGAState *)vga);
    }
	...
	vga_dirty_log_start((VGAState *)vga);
    }
}
...
static void qxl_reset_handler(void *dev)
{
    // PCIQXLDevice *d = DO_UPCAST(PCIQXLDevice, pci.qdev, dev);
    qxl_hard_reset((PCIQXLDevice *)dev, 0);
}
...
static void qxl_add_memslot(PCIQXLDevice *d, uint32_t slot_id, uint64_t delta)
{
    ...
    case QXL_RAM_RANGE_INDEX:
        ...
        virt_start = (intptr_t)(d->vga.vram_offset);
        break;
    case QXL_VRAM_RANGE_INDEX:
        //virt_start = (intptr_t)qemu_get_ram_ptr(d->vram_offset);
        virt_start = (intptr_t)(d->vram_offset);
        break;
    default:
        /* should not happen */
        abort();
...
static void qxl_ioport_write(void *opaque, uint32_t addr, uint32_t val)
{
...
static uint32_t qxl_ioport_read(void *opaque, uint32_t addr)
{
    PCIQXLDevice *d = opaque;
                    ...
                    pcibus_t addr, pcibus_t size, int type)
{
    ...
    case QXL_IO_RANGE_INDEX:
        register_ioport_write(addr, size, 1, qxl_ioport_write, pci);
        register_ioport_read(addr, size, 1, qxl_ioport_read, pci);
        qxl->io_base = addr;
        break;
    case QXL_RAM_RANGE_INDEX:
              ...
              uint32_t vga_ram_size)
{
    ...
    //VGACommonState *vga;
    VGAState *vga;
    ram_addr_t ram_size; // vga ram size
    uint32_t rom_size;
    uint32_t max_fb;
    ...
    PCIQXLDevice *qxl = (PCIQXLDevice*)pci_register_device(bus, QXL_DEV_NAME,
                                        sizeof(PCIQXLDevice), -1, NULL,
                                       NULL);
    opaque_qxl = qxl;

    if (!qxl) {
      fprintf(stderr, ":%s:qxl is NULL\n", __FUNCTION__);
      return -1;
    }
    
    qxl->pci.romfile = NULL;
    qxl->vga.vram_size = vga_ram_size; //vga_ram_size;
    qxl->vram_size = 64 * 1024 * 1024; // pci bar
    qxl->revision = 2;
    qxl->debug = 0;
    qxl->guestdebug = 0;
    qxl->cmdlog = 0;

    config = qxl->pci.config;
    //ram_size = msb_mask(qxl->vga.vram_size * 2 - 1);
    ram_size = msb_mask(vga_ram_size * 2 - 1); 
	...
	vga_common_init1(vga, vga_ram_base, vga_ram_offset, ram_size);
        vga_init1(vga);
        //vga_init2(vga); // need to rewrite here.
        ...
        if (!(qxl->ssd.buf)) {
            fprintf(stderr, ":%s:qxl->ssd.buf=qemu_malloc is NULL\n", __FUNCTION__);
            return -1;
        }
        ...
        //                                      qxl->vga.vram_size); // qemu_ram_alloc change dramaticly
        //qxl->vga.vram_offset = qemu_ram_alloc(qxl->vga.vram_size);
        //qxl->vga.vram_offset = qemu_malloc(qxl->vga.vram_size);
#if 0 // skylark
        qxl->vga.vram_offset = qemu_vmalloc(qxl->vga.vram_size);
 
        //qxl->vga.vram_ptr = qemu_get_ram_ptr(qxl->vga.vram_offset);
        qxl->vga.vram_ptr = qxl->vga.vram_offset;
#endif
    ...
    config[0x08] = pci_device_rev;
    config[0x3d] = 1; // Interrupt pin
    ...
    //if (qxl->vram_size < 4 * 1024 * 1024) {
    //    qxl->vram_size = 4 * 1024 * 1024;
    //}
    if (qxl->revision == 1) {
        qxl->vram_size = 4096;
    }
    ...
    */
    ...
    pci_register_io_region(&qxl->pci, QXL_IO_RANGE_INDEX,
                           io_size,
                           PCI_ADDRESS_SPACE_IO, qxl_map);

    pci_register_io_region(&qxl->pci, QXL_ROM_RANGE_INDEX,
                           qxl->rom_size, PCI_ADDRESS_SPACE_MEM,
                           qxl_map);

    pci_register_io_region(&qxl->pci, QXL_RAM_RANGE_INDEX,
                           qxl->vga.vram_size, PCI_ADDRESS_SPACE_MEM,
                           qxl_map);

    pci_register_io_region(&qxl->pci, QXL_VRAM_RANGE_INDEX, qxl->vram_size,
                           PCI_ADDRESS_SPACE_MEM_PREFETCH, qxl_map);
...
{
    PCIQXLDevice *qxl = (PCIQXLDevice *)opaque;
    qxl->ssd.qxl.base.sif = &qxl_interface.base;
    qxl->ssd.qxl.id = qxl->id;
    qemu_spice_add_interface(&qxl->ssd.qxl.base);
    qemu_add_vm_change_state_handler(qxl_vm_change_state_handler, qxl);
...
#if 0 // qemu-0.13.55 commented, need to rewrite for qemu-0.10.2
...
#endif // commented, need to rewrite for qemu-0.10.2
